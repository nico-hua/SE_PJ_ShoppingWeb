<template>
  <el-container>
    <el-header><el-menu mode="horizontal" :ellipsis="false" router default-active="/modifygoodshandler">
    ...
    <el-main class="main">
      ...
      <el-space wrap>
        <el-card v-for="good in goods" :key="good.goodId" class="box-card" style="width: 250px">
          ...
          <div class="picture" v-if="good.imageString && good.imageString.length > 0">
          ...
          <div class="card-footer">
            ...
            <el-button class="button" @click="disapprove(good.id)" type="danger" plain>驳回</el-button>
          </div>
        </el-card>
      </el-space>
    </el-main>
...
export default {
  ...
  mounted() {
    ...
    this.$axios
      .get('/admin/showAllCommodityFixedToBeReviewed', {
        headers: {
          token: `${token}`
        }
      })
  ...
  methods: {
    approve(goodId) {
      console.log(`批准修改商品 ${goodId}`)
      const token = localStorage.getItem('token')
      this.$axios
        .post(
          '/admin/agreeFixCommodity',
          { id: goodId },
          {
            headers: {
              token: `${token}`,
    ...
    disapprove(goodId, imageUrl) {
      console.log(`驳回修改商品 ${goodId}`)
      const token = localStorage.getItem('token')
      this.$axios
        .post(
          '/admin/refuseFixCommodity',
          { id: goodId },
          {
