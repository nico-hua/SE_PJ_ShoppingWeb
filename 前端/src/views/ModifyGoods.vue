<template>
  <el-container>
    <el-header><el-menu mode="horizontal" :ellipsis="false" router>
    ...
    <el-main><el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item>
          <el-button type="primary" @click="handleSubmit">提交修改申请</el-button>
...
export default {
  ...
  mounted() {
    ...
    this.$axios
      ...
      .then((response) => {
        console.log(response.data)
        this.ruleForm.name = response.data.data.commodityName
        this.ruleForm.price = response.data.data.price
        this.ruleForm.description = response.data.data.introduction
        this.ruleForm.type = response.data.data.categoryName
      })
      .catch((error) => {
        console.log(error)
      })
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      this.formData.set(' commodityId', this.$route.params.id)
      this.$axios
        .post('/merchant/fixCommodity', this.formData, {
          headers: {
            token: `${token}`,
